...
function switchTab(tab) {
    ...
    if (tab === "live") { refreshLive(); liveTimer = setInterval(refreshLive, 3000); }
...
// ══════════════════════════════════════════════════════════
async function refreshLive() { window._refreshLiveActive = true; await _refreshLiveInner(); }
async function _refreshLiveInner() {
    // 2026-09 redesign: the Live tab is rendered by static/js/live.js from
    // the same endpoints. The paper summary writer stays for the other tabs.
    try { if (window.LiveTab) await window.LiveTab.refresh(); } catch (e) { console.warn("LiveTab.refresh", e); }
