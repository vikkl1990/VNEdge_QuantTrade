...
class ScannerWeightManager:
    ...
    #
    # (2026-09-13) Emptied at the user's explicit direction after asking
    # which scanner(s) to reactivate and being told "all 18, full revert" --
    # not because the evidence below was wrong. It stands:
    #
    ...
    # and hoped-for on the other. 17 of 18 failed: negative in at least one
    # fold, or flipped sign between them -- the same overfitting pattern
    # structure_bounce showed on three separate hand-tuned rebuilds this
    # session (supertrend_flip and momentum_surge were already shadowed
    # before this gate, on live P&L: 26% WR/-9.62R and 39% WR/-0.41R).
    # rsi_divergence was the one exception, statistically real (not noise),
    # but a same-methodology confidence-threshold follow-up
    # (scratchpad/rsi_div_confidence_gate.py, 47,789 candidates) found no
    # confidence cut where its edge clears round-trip fees: 0.15-0.20 ATR
    # raw edge against a 0.62-0.73 ATR fee cost at every band, in both
    # folds. Net finding as of this revert: 0 of 18 routed scanners had
    # demonstrated a fee-clearing, fold-consistent edge.
    #
    # Every scanner is tradeable again from here. Re-run
    # scratchpad/scanner_gate.py (or the confidence-threshold variant for a
    # single scanner) for the same evidence-based basis to shadow one again.
