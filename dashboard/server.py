...
class DashboardServer:
    ...
    def _register_routes(self, app: web.Application) -> None:
        ...
        # Scanner Lab (2026-09-17): per-pair scanner x timeframe replay written
        # by scripts/scanner_lab.py; rendered by /static/scanner_lab.html
        app.router.add_get("/api/research/scanner-lab/symbols", self._handle_scanner_lab_symbols)
        app.router.add_get("/api/research/scanner-lab", self._handle_scanner_lab)
        ...
        # Perp-terminal live view + operator actions (2026-09-20)
        app.router.add_get("/api/live/position", self._handle_live_position)
        app.router.add_get("/api/live/context", self._handle_live_context)
        app.router.add_get("/api/ledger/fees", self._handle_fee_ledger)
        app.router.add_get("/api/chart/overlay", self._handle_chart_overlay)
        app.router.add_post("/api/trade/{trade_id}/close", self._handle_trade_close)
        app.router.add_post("/api/trade/{trade_id}/close-partial", self._handle_trade_close_partial)
        app.router.add_post("/api/trade/{trade_id}/stop", self._handle_trade_stop)
        app.router.add_post("/api/control/close-all", self._handle_close_all)
    ...
    async def _handle_validated_edge(self, request: web.Request) -> web.Response:
        ...
        edge test, not a live backtest claim) from docs/validation/ — a
        committed research artifact, not runtime state, so it lives outside
        ...
        try:
            return web.json_response(json.loads(path.read_text()), dumps=_safe_dumps)
        except Exception as e:
            return web.json_response({"error": str(e)}, status=500)

    # Scanner Lab — offline per-pair replay output (scripts/scanner_lab.py).
    # Same shape as _handle_validated_edge above: a file on disk, read fresh
    # per call (regenerated by hand, small enough not to cache).
    _SCANNER_LAB_DIR = _DASHBOARD_DIR.parent / "storage" / "research" / "scanner_lab"

    async def _handle_scanner_lab_symbols(self, request: web.Request) -> web.Response:
        out = []
        if self._SCANNER_LAB_DIR.is_dir():
            for p in sorted(self._SCANNER_LAB_DIR.glob("*.json")):
                try:
                    meta = json.loads(p.read_text()).get("meta", {})
                except Exception:
                    continue
                out.append({"symbol": meta.get("symbol", p.stem.replace("_", "/", 1)),
                            "generated_at": meta.get("generated_at"),
                            "timeframes": meta.get("timeframes", []),
                            "days": meta.get("days")})
        return web.json_response(out, dumps=_safe_dumps)

    async def _handle_scanner_lab(self, request: web.Request) -> web.Response:
        symbol = (request.query.get("symbol") or "BTC/USDT").upper()
        stem = re.sub(r"[^A-Z0-9_]", "", symbol.replace("/", "_"))
        path = self._SCANNER_LAB_DIR / f"{stem}.json"
        if not stem or not path.is_file():
            return web.json_response(
                {"error": f"no scanner lab run for {symbol}",
                 "hint": f"python scripts/scanner_lab.py --symbol {symbol}"}, status=404)
        try:
            return web.json_response(json.loads(path.read_text()), dumps=_safe_dumps)
        except Exception as e:
            return web.json_response({"error": str(e)}, status=500)
    ...
    def _live_prices(self) -> Dict[str, float]:
        """Fresh prices only (WS tick < 90 s, else candle < 12 min); falls back to the last push."""
        orch = getattr(self, "_orchestrator", None)
        try:
            if orch is not None and hasattr(orch, "_fresh_prices"):
                fresh = orch._fresh_prices()
                if fresh:
                    return dict(fresh)
        except Exception:
            pass
    ...
    def _ticker_meta(self) -> Dict[str, Dict[str, Any]]:
        orch = getattr(self, "_orchestrator", None)
        ws = getattr(orch, "_delta_ws", None) if orch is not None else None
        try:
            return ws.get_ticker_meta() if ws is not None else {}
        except Exception:
            return {}

    def _enriched_positions(self) -> List[Dict[str, Any]]:
        if not self._signal_tracker:
            return []
        from bot.live_view import enrich_position
        from execution.fees import get_fee_model
        fm = get_fee_model()
        prices, meta = self._live_prices(), self._ticker_meta()
        rows = []
        for t in self._signal_tracker.get_active_signals():
            m = meta.get(t.get("symbol")) or {}
            mark = m.get("mark") or prices.get(t.get("symbol"))
            try:
                rows.append(enrich_position(t, mark, m, fm))
            except Exception as exc:
                rows.append({"trade_id": t.get("trade_id"), "symbol": t.get("symbol"), "error": str(exc)[:200]})
        return rows

    async def _handle_live_position(self, request: web.Request) -> web.Response:
        rows = self._enriched_positions()
        max_open = 3
        try:
            max_open = int((self._config_risk() or {}).get("max_open_positions", 3))
        except Exception:
            pass
        return web.json_response({"trades": rows, "n": len(rows), "max_open": max_open,
                                  "ws_clients": self._ws_hub_clients(), "ts": datetime.now(timezone.utc).isoformat()},
                                 dumps=_safe_dumps)

    def _ws_hub_clients(self) -> int:
        try:
            from dashboard.websocket_handler import hub
            return hub.client_count
        except Exception:
            return 0

    async def _handle_live_context(self, request: web.Request) -> web.Response:
        from bot.live_view import market_context
        return web.json_response(market_context(self._ticker_meta(), self._live_prices()), dumps=_safe_dumps)

    async def _handle_fee_ledger(self, request: web.Request) -> web.Response:
        from bot.live_view import fee_ledger
        from execution.fees import get_fee_model
        try:
        ...
        closed = self._signal_tracker.get_closed_signals(limit=5000) if self._signal_tracker else []
        if days:
            try:
                cutoff = datetime.now(timezone.utc).timestamp() - float(days) * 86400
                closed = [t for t in closed if t.get("exit_time") and
                          datetime.fromisoformat(str(t["exit_time"]).replace("Z", "+00:00")).timestamp() >= cutoff]
            except Exception:
                pass
        return web.json_response(fee_ledger(closed, get_fee_model(), limit=limit), dumps=_safe_dumps)

    async def _handle_chart_overlay(self, request: web.Request) -> web.Response:
        from bot.live_view import chart_overlay
        symbol = request.query.get("symbol", "BTC/USDT")
        interval = request.query.get("interval", "5m")
        try:
            limit = max(50, min(1000, int(request.query.get("limit", "300"))))
            k = max(0.5, min(6.0, float(request.query.get("k", "2.0"))))
        except Exception:
            limit, k = 300, 2.0
        if self._data_manager is None:
            return web.json_response({"error": "data_manager not ready"}, status=503)
        try:
            ...
            if df is None or len(df) == 0:
                return web.json_response({"error": f"no cached candles for {symbol} @ {interval}"}, status=404)
            active = self._signal_tracker.get_active_signals() if self._signal_tracker else []
            closed = self._signal_tracker.get_closed_signals(limit=500) if self._signal_tracker else []
            return web.json_response(chart_overlay(df, symbol, active, closed, k=k), dumps=_safe_dumps)
        except Exception as e:
            return web.json_response({"error": str(e)[:300]}, status=500)

    async def _action_body(self, request: web.Request) -> dict:
        try:
            body = await request.json()
    ...
    def _action_context(self, trade_id: str):
        """(tracker, trade dict, fresh price) or an error response."""
        if not self._signal_tracker:
            return None, None, None, web.json_response({"ok": False, "error": "tracker not ready"}, status=503)
        t = next((x for x in self._signal_tracker.get_active_signals() if x.get("trade_id") == trade_id), None)
        if t is None:
            return None, None, None, web.json_response({"ok": False, "error": "trade not active"}, status=404)
        px = self._live_prices().get(t.get("symbol"))
        if not px:
            return None, None, None, web.json_response({"ok": False, "error": f"no fresh price for {t.get('symbol')}"}, status=409)
        return self._signal_tracker, t, float(px), None

    async def _after_action(self, result: dict, kind: str, t: dict) -> None:
        await self.add_alert("warning" if kind in ("close", "close-all") else "info",
                             f"Operator {kind}: {t.get('symbol')} {t.get('side')} — {result.get('error') or 'ok'}", source="dashboard")
        try:
            from dashboard.websocket_handler import hub
            await hub.push_position(self._enriched_positions(), force=True)
            await hub.push_events([{"type": f"operator_{kind}", "message": f"{kind} {t.get('symbol')}", "signal": t}])
        except Exception:
            pass

    async def _handle_trade_close(self, request: web.Request) -> web.Response:
        tid = request.match_info["trade_id"]
        tracker, t, px, err = self._action_context(tid)
        if err:
            return err
        body = await self._action_body(request)
        res = tracker.close_trade(tid, px, reason=str(body.get("reason") or "manual_close")[:40], by="dashboard")
        await self._after_action(res, "close", t)
        return web.json_response(res, dumps=_safe_dumps, status=200 if res.get("ok") else 409)

    async def _handle_trade_close_partial(self, request: web.Request) -> web.Response:
        tid = request.match_info["trade_id"]
        tracker, t, px, err = self._action_context(tid)
        if err:
            return err
        body = await self._action_body(request)
        try:
            frac = float(body.get("fraction", 0.5))
        except Exception:
            frac = 0.5
        res = tracker.close_partial(tid, frac, px, by="dashboard")
        await self._after_action(res, "close-partial", t)
        return web.json_response(res, dumps=_safe_dumps, status=200 if res.get("ok") else 409)

    async def _handle_trade_stop(self, request: web.Request) -> web.Response:
        tid = request.match_info["trade_id"]
        tracker, t, px, err = self._action_context(tid)
        if err:
            return err
        body = await self._action_body(request)
        if body.get("breakeven") or body.get("price") in (None, "", "breakeven"):
            res = tracker.move_stop_to_breakeven(tid, px, by="dashboard")
        else:
            try:
                res = tracker.set_stop(tid, float(body.get("price")), px, by="dashboard")
            except Exception:
                res = {"ok": False, "error": "price must be a number"}
        await self._after_action(res, "stop", t)
        return web.json_response(res, dumps=_safe_dumps, status=200 if res.get("ok") else 409)

    async def _handle_close_all(self, request: web.Request) -> web.Response:
        """Flatten: close every active paper trade at the fresh price and pause new entries."""
        if not self._signal_tracker:
            return web.json_response({"ok": False, "error": "tracker not ready"}, status=503)
        body = await self._action_body(request)
        res = self._signal_tracker.close_all(self._live_prices(), reason=str(body.get("reason") or "flatten")[:40], by="dashboard")
        if body.get("pause", True):
            async with self._lock:
                self._paused = True
                self._bot_status = "paused"
            res["paused"] = True
        logger.warning("Operator close-all via dashboard: %s", res)
        await self._after_action(res, "close-all", {"symbol": "ALL", "side": f"{res.get('closed')}/{res.get('n')}"})
        return web.json_response(res, dumps=_safe_dumps, status=200 if res.get("ok") else 409)

    # -- push helpers called by the orchestrator tick ----------------------
    async def push_tick(self, prices: Dict[str, float]) -> None:
        try:
            from dashboard.websocket_handler import hub
            if hub.client_count == 0:
                return
            meta = self._ticker_meta()
            marks = {s: m.get("mark") for s, m in meta.items() if m.get("mark")}
            await hub.push_tick(prices, marks)
            if self._signal_tracker and self._signal_tracker.active_count > 0:
                await hub.push_position(self._enriched_positions())
        except Exception as exc:
            logger.debug("push_tick failed: %s", exc)

    async def push_events(self, events: list) -> None:
        try:
            from dashboard.websocket_handler import hub
            if hub.client_count == 0 or not events:
                return
            await hub.push_events(events)
            await hub.push_position(self._enriched_positions(), force=True)
        except Exception as exc:
            logger.debug("push_events failed: %s", exc)
