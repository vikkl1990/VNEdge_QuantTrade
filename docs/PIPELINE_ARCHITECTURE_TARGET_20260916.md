...
Nothing in "Concrete changes" has been started. This doc is the shared reference for when implementation is explicitly scoped. See `SCANNER_CLUSTER_ANALYSIS_TODO_20260915.md` for the specific findings this synthesizes and for the still-open per-scanner decisions (`bos_choch` second gate, `ema_momentum` quarantine, `vwap_band` exclusion, forced-RUNNER override, reversion-tuple membership, `classify_trade` 0.50/0.65) that item 5 and item 1 above would resolve as a side effect of the broader contract, but that remain independently open regardless of when/whether this refactor happens.
