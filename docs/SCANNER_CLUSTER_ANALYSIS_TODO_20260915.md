...
## Scanner #15 added: `fvg_fill` (Fair Value Gap), 2026-09-17

New scanner, requested after reviewing an "institutional price action" reference sheet against the bot's existing coverage — FVG/imbalance detection was the clearest gap (BOS/CHOCH, order blocks, and liquidity sweeps already existed and had been tested; FVG did not, though the core 3-candle gap math already existed as an *ML training feature* in `ml_training/feature_builder.py`, never wired into the live path). Planned via `EnterPlanMode` with two research passes (scanner-wiring pattern, using `volume_surge`/`candlestick_reversal` — the two most recently added scanners — as the up-to-date template; structure-map generator pattern, using `detect_order_blocks()` as the closest precedent) before writing any code.

- **`data/structure.py`**: new `find_fair_value_gaps()` generator, modeled directly on `detect_order_blocks()` — 3-candle gap check (`lows[i] > highs[i-2]` bullish / mirrored bearish), `0.15×ATR` minimum gap size (vs. OB's `1.5×ATR` impulse — FVGs are inherently subtler), identical unbounded-mitigation policy to order blocks (deliberately reused, not re-derived, to keep the generator family internally consistent — OB's unbounded mitigation is itself an open "zone lifetime policy" question from earlier this session, not being re-litigated here). Wired into `build_structure_map()` alongside the other 3 non-VWAP generators, no schema changes needed on `StructureLevel`/`StructureMap`.
- **`strategies/scalp_strategy.py`**: new `_scan_fvg_fill`, modeled directly on `_scan_order_block_entry` (same "return to unmitigated zone + directional confirmation" mechanic). Full wiring checklist completed: `SCANNER_CLUSTER["fvg_fill"]="structure"`, `_PAPER_UNVALIDATED_SCANNERS` (quarantined like `volume_surge`/`candlestick_reversal` — can't outrank a live-validated same-cluster sibling on score alone), routed to `breakout`/`ranging`/`sideways`/`volatile`/`high_volatility` (mirrors `order_block_entry`'s own regime membership, the closest analog), `scanner_names`, `_all_scanner_map`. Unlike `volume_surge`/`candlestick_reversal` (confirmed to have shipped with no shadow-diagnostic entry, falling through to `""`), `fvg_fill` got a real diagnostic from day one — that omission was a recurring gap this session kept having to fix after the fact for other scanners; not repeating it on a new one. No `_scanner_sl_tp` entry (matches the same two scanners' precedent — falls back to the confirmed-safe global defaults, `sl_atr=2.0`/`tp1_rr=1.5`/`tp2_rr=2.5`/`tp3_rr=4.0`). Standard `classify_trade()`-based exit ladder, no `SCANNER_TRADE_TYPE` override.
- **Tests**: no scanner-level unit-test convention exists anywhere in this codebase (confirmed by research before assuming one) — added `tests/test_structure_fvg.py` instead, 6 synthetic-data tests on the pure generator function (bullish/bearish detection, mitigation exclusion, sub-noise-floor rejection, empty/short-df edge cases), same pattern used for the liquidity-zone dedup fix earlier this session. Hit the same synthetic-data pitfall as that earlier fix on the first pass — a trailing return-to-baseline in the test data read as its own unintended second gap — fixed by keeping test price series from snapping back near the leading baseline rather than by loosening the assertions.
